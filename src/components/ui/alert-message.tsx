...
export default function AlertMessage({ status, message }: { status: boolean; message: string }) {
  ...
  return (
    <Alert
      className={cn(
        "h-12 w-96 flex items-center fixed bottom-6 -right-96 rounded-none bg-gray-800 text-white shadow-xl transition-right duration-200",
        isShow && "right-6"
      )}
    >
      <AlertTitle className={cn("m-0 flex items-center gap-3")}>
        <CheckIcon /> {message}
      </AlertTitle>
    </Alert>
  );
