...
export default async function AsidebarRoot() {
       ...
       <Asidebar className="max-w-asidebar-width">
              ...
              <Link className={cn("logo")} href="/">
                     ...
                     <p className={cn("mt-2 flex items-center gap-2 text-xl font-semibold text-blue-600")}><FolderCodeIcon /> Exam App</p>
              </Link>

              {/* body-links asidebar part */}
              <ul className={cn("links | flex-auto gap-2")} aria-label="list links">
                     <AsideLink href="/"><GraduationCapIcon /> Diplomas</AsideLink>
                     <AsideLink href="/account"><UserAccountIcon /> Account Settings</AsideLink>
              </ul>

              {/* footer asidebar part */}
              <div className={cn("user | relative flex items-center gap-2")} aria-label="user">
                     <Image src={session?.user?.image ?? "/images/avatar.webp"} alt="photo user" width="54" height="54" loading="lazy" />
                     <div className="info">
                            <span className={cn("text-blue-600 font-semibold")} aria-label="first name">{session?.user?.name}</span>
                            <p className={cn("max-w-44 text-sm overflow-hidden text-ellipsis")} aria-label="email user">{session?.user?.email}</p>
                     </div>
                     <ButtonDropdown /> 
              </div>
       </Asidebar>
