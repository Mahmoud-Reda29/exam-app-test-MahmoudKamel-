...
export default function Diplomas() {
       const { data, error, isLoading, hasNextPage, fetchNextPage, isFetchingNextPage } = useInfiniteQuery<GetAllSubjects>({
              queryKey: ["subjects"],
       ...
       if (error || !data) 
              return <p className={cn("mt-10 text-red-500 text-center font-bold")}>
       ...
       >
              {data?.pages.flatMap(page => 
                     ...
                     <Link key={item._id} href="/exams" className="item | h-card-height relative overflow-hidden">
                            <Image
