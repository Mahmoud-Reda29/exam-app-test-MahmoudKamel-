...
export default function VerifyOTP({ onSuccess, onBack, getEmail }: ForgotPasswordAuthProps & { getEmail: string }) {
       ...
       const { control, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormInput<typeof VerifyOTPSchema>>({ resolver: zodResolver(VerifyOTPSchema) });
       ...
       >
              ...
              {/* title */}
              <h1
                     className={cn("text-3xl text-gray-800 font-bold", inter.className)}
              >
                     Verify OTP
              </h1>
              ...
              <p className="mb-7 text-gray-500">
                     ...
                     <span className={cn("text-gray-800 font-medium")}>{getEmail} <button role="link" type="button" onClick={onBack} className={cn("text-blue-600 underline")}>Edit</button></span>
              </p>
                            ...
                            >
                                   <InputOTPGroup className="gap-4 !rounded-none">
                                          {Array.from({ length: 6 }).map((_, i) => (
                                                 <InputOTPSlot key={i} className="h-10 w-10 !rounded-none border-gray-200" index={i} />
                                          ))}
                                   </InputOTPGroup>
                            </InputOTP>
                     )}
              ...
              {/* when error */}
              { errors.resetCode && <p className="invalid | text-sm text-red-600">{ errors.resetCode.message }</p>}
