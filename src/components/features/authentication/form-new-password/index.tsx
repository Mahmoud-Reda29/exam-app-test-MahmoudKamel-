...
export default function FormNewPassword({
  ...
  } = useForm<FormInput<typeof NewPasswordSchema>>({
    resolver: zodResolver(NewPasswordSchema),
    defaultValues: { email: getEmail },
  });
  ...
  return (
    ...
    >
      <h1 className={cn("text-3xl text-gray-800 font-bold")}>Create a New Password</h1>
      <p className={cn("mb-7 ")}>Create a new strong password for your account.</p>

      <div
        className={cn("new-password | w-full mb-4 flex flex-col gap-2 text-gray-800 font-medium")}
      >
        ...
        />
        {errors.newPassword && (
          <p className="invalid | text-sm text-red-600">{errors.newPassword.message}</p>
        )}
      </div>

      <div
        className={cn(
          "confirm-new-password | w-full mb-4 flex flex-col gap-2 text-gray-800 font-medium"
        )}
      >
        ...
        {errors.confirmPassword && (
          <p className="invalid | text-sm text-red-600">{errors.confirmPassword.message}</p>
        )}
      </div>
