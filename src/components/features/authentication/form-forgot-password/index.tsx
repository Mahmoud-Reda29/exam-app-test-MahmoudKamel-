...
export default function FormForgotPassword({onSuccess, setEmail} : ForgotPasswordAuthProps & {setEmail:(email:string)=> void}) {
       ...
       const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormInput<typeof ForgotPasswordSchema>>({resolver: zodResolver(ForgotPasswordSchema)});
       ...
       >
              ...
              {/* Title for the form */}
              <h1 className={cn("text-3xl text-gray-800 font-bold")}>
                     Forgot Password
              </h1>


              {/* Form description */}
              <p className="mb-7">Don’t worry, we will help you recover your <br /> account.</p>


              {/* Form input for the user's email  */}
              <div className={cn("email | w-full mb-4 flex flex-col gap-2 text-gray-800 font-medium")}>
                     <Label htmlFor="email">Email</Label>
                     <Input className="h-12 border-gray-200 placeholder:text-gray-400 focus:border-blue-600 transition-border duration-200" type="text" id="email" {...register("email")} placeholder="user@example.com" />
              ...
              <button 
                     className={cn("w-full mt-10 p-3 flex items-center gap-2 justify-center bg-blue-600 text-white font-sm font-medium hover:bg-blue-700 transition-background-color duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-blue-600")} 
                     disabled={isReady || isSubmitting}
              >
                     {isSubmitting ? <ButtonLoading /> : <>Continue <MoveRightIcon /></>}
              </button>
              ...
              <p className={cn("w-full mt-9 text-center text-lg text-gray-500")}>
                     ...
                     <Link className="text-blue-600 text-lg" href="/auth/register">Create yours</Link>
              </p>
       </form>
       </>
       );
